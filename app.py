...
def annual_to_monthly_rate(annual_rate_pct: float, convention: str = "Nominal (Annual / 12)") -> float:
    """Converts an annual rate to a monthly rate.

    Nominal (Annual / 12): the convention most commonly used in Indian lease
    workings (e.g. 8.25% p.a. -> 0.6875% per month). This is the default.

    Effective (compounded monthly): (1+r)^(1/12) - 1.
    """
    r = annual_rate_pct / 100
    if convention.startswith("Nominal"):
        return r / 12
    return (1 + r) ** (1 / 12) - 1
...
def lease_liability_schedule(payments, monthly_rate, timing, start_date):
    ...
    Arrears (payment at end of month): standard textbook amortization —
    interest accrues first on the opening balance, then the payment is
    deducted.

    Advance (payment at start of month, i.e. annuity-due): the payment made
    on the commencement date itself attracts NO interest (no time has
    elapsed), so Month 1 shows zero finance cost. From Month 2 onward,
    interest accrues on the balance carried forward after the prior month's
    payment, and is added before that month's payment is deducted. This
    matches standard Ind AS 116 amortization tables built for rent payable
    in advance.
    """
    ...
    if timing == "Advance (start of month)":
        bal = opening_bal - payments[0]
        rows.append({
            "Month": 1,
            "Date": start_date.strftime("%b-%Y"),
            "Opening Liability": round(opening_bal, 2),
            "Interest / Finance Cost": 0.0,
            "Lease Payment": round(payments[0], 2),
            "Closing Liability": round(bal, 2),
        })
        for i in range(2, len(payments) + 1):
...
def deposit_unwinding_schedule(deposit_amount, market_rate_pct, deposit_term_months, start_date,
                                rate_convention="Nominal (Annual / 12)"):
    """Unwinding of discount on interest-free refundable security deposit."""
    monthly_rate = annual_to_monthly_rate(market_rate_pct, rate_convention)
...
with st.sidebar:
    ...
    rate_convention = st.selectbox(
        ...
        ["Nominal (Annual / 12)", "Effective (compounded monthly)"],
        help="Nominal (Annual/12) is the convention most commonly used in Indian lease "
             "workings — e.g. 8.25% p.a. becomes 0.6875% per month. Effective compounds "
             "the annual rate monthly instead.",
    )
...
monthly_rate = annual_to_monthly_rate(discount_rate, rate_convention)
...
if has_deposit and fair_value_deposit and deposit_amount > 0 and deposit_term_years > 0:
    deposit_df, pv_deposit, day1_diff = deposit_unwinding_schedule(
        deposit_amount, market_rate, int(round(deposit_term_years * 12)), start_date, rate_convention
    )
